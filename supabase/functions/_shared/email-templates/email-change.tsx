...
export const EmailChangeEmail = ({ siteName, email, newEmail, confirmationUrl }: EmailChangeEmailProps) => (
  <Html lang="no" dir="ltr">
    <Head />
    <Preview>Bekreft endring av e-postadresse for Snakk</Preview>
    <Body style={main}>
      <Container style={container}>
        ...
        <Section style={contentSection}>
          <Heading style={h1}>Bekreft e-postendring</Heading>
          <Text style={text}>
            Du har bedt om å endre e-postadressen din for Snakk fra{' '}
            <Link href={`mailto:${email}`} style={link}>{email}</Link>{' '}til{' '}
            <Link href={`mailto:${newEmail}`} style={link}>{newEmail}</Link>.
          </Text>
          <Text style={text}>Klikk på knappen under for å bekrefte endringen:</Text>
          <Button style={button} href={confirmationUrl}>
            Bekreft e-postendring
          </Button>
          <Text style={footer}>
            Hvis du ikke ba om denne endringen, vennligst sikre kontoen din umiddelbart.
          </Text>
        </Section>
