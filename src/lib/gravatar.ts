 ...
 * Uses a simple MD5 implementation to avoid adding a dependency.
 */
...
function md5(input: string): string {
  ...
  function binlSafeAdd(x: number[], y: number[]): number[] {
  ...
  if (n > 55) {
    // need two blocks – simplified, won't happen for emails
  }
  ...
  for (let j = 0; j < 4; j++) {
    for (let k = 0; k < 4; k++) {
      ...
      s += hex.charAt((byte >> 4) & 0x0f) + hex.charAt(byte & 0x0f);
    }
  }
  return s;
}
 ...
 * Falls back to "mp" (mystery person) if no Gravatar exists.
 */
